...
## notify (unreleased)
- FEATURE: `INotifyWatcher` tracks a `TrackPath` watch through every ancestor: the watched path is reported as removed when a directory above it is moved away or deleted, reported as created and watched again when it is reachable again (a directory without its content), and a path below directories that do not exist yet can be watched. On inotify, kqueue and Windows an ancestor that cannot be examined makes `watch` fail instead of waiting for the path, an ancestor above the parent that can be examined but not watched is skipped, and watching a path again re-arms one whose last arm failed; on inotify a directory reached through several spellings (a symlink and its real path) is reported under each spelling that a watched path covers [#32]
- FEATURE: `FsEventWatcher` reports a `TrackPath` watch as removed and as created when a directory above it goes and comes back, also for a path FSEvents does not report itself: one folded into an ancestor with other watched paths (ten or more in one directory, or more than `Config::with_max_fsevent_paths` in all), one below another watched path, or one below directories that do not exist yet (while it is missing, the deepest existing ancestor that is at least three directories deep is watched recursively for it, the watched paths below that ancestor are folded into it, and the ancestor is dropped at the first `watch`, `unwatch` or commit after the path or its parent appears). The root's coming back is reported by whichever side sees it first, since FSEvents does not report a root that comes back within a few milliseconds and sends the root's own creation a moment later; a path that is not a stream path of its own reports its own removal only, while a stream path may report its own removal and a `root changed` removal. A `NoTrack` watch gets none of this, and one folded into such an ancestor is no longer reported removed when a directory above it moves. Known limits: paths that are not a stream path of their own may not all be reported when a directory above them is swapped for another or goes and comes back within a few milliseconds, and once the directories above a path were deleted two or more levels deep and restored while watching, their later moves are not reported until the next `watch`, `unwatch` or commit [#32]
- FEATURE: `KqueueWatcher` tracks a `TrackPath` watch through every ancestor like `INotifyWatcher`, follows an ancestor that is a symlink to a directory (when the link is removed and created again later; a link replaced at once, as `ln -sfn` does, or a move of the directory it points to is not noticed), and removes the watches below a moved or deleted directory by the watches it holds instead of walking the disk, which left them on the moved files [#32]
- FEATURE: `ReadDirectoryChangesWatcher` tracks a `TrackPath` watch through every ancestor, watches a tracked directory once it appears, reports a path that comes back but cannot be opened as an error and arms it again when it is watched again, and fails only the `watch` or commit entry whose own directory cannot be opened [#32]
- FIX: `INotifyWatcher` and `KqueueWatcher` keep the watches that other watched paths still need when a path is unwatched: a directory above other watched paths stays watched as their ancestor, the paths below an unwatched recursive directory stay watched, and a file that was reported through its parent keeps being reported [#32]
- FIX: `KqueueWatcher` no longer drops a recursive `NoTrack` watch when a directory is created or removed in it, and unwatching a path that is not there releases the watches on its ancestors [#32]
