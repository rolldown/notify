...
- FEATURE: `FsEventWatcher` reports a `TrackPath` watch as created when it is reachable again after a directory above it was moved away, instead of dropping the root change; a root that went is looked at again shortly after, since FSEvents does not report one that comes back within a few milliseconds [#32]
